...
def test_valid_majority_passes(monkeypatch, capsys):
    ...
    monkeypatch.setattr(gate_check, "propose", lambda phrasing, **_kwargs: _result(call))
...
def test_minority_proposals_fail(monkeypatch, capsys):
    ...
    monkeypatch.setattr(gate_check, "propose", lambda phrasing, **_kwargs: next(results))
...
def test_malformed_propose_result_is_not_ignored(monkeypatch):
    gate_check = _load_gate_check()
    monkeypatch.setattr(
        gate_check, "propose", lambda phrasing, **_kwargs: (None, {"content": ""}, "context")
    )
