"""Typed operational failures that may safely cross the resolver boundary.

P08 deliberately distinguishes an unavailable authoritative source from a
programming error.  Only these explicit signals are eligible for manifest
fail-posture handling; arbitrary exceptions remain loud.
"""
...
class AmbiguousPolicyState(RuntimeError):
    ...
    def __init__(self, *, policy_id: str, row_count: int, query: str) -> None:
        ...
        self.query = query
        super().__init__(
            f"policy {policy_id!r} has {row_count} rows marked current"
        )
