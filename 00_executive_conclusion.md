...
At the audited state, it was **not a research artifact**, and the gap was not one of polish. It was that **every headline number in the repository was analytically forced by its own generator**, and the one artifact that would break that circularity — `bench/label.py`, the independent labeller — **was documented but not committed**.
...
**It is not true that "no additional experiment is justified."** Exactly one is: the gold-set-scored confusion matrix. At audit time, branch `8a48bf7` contained partial progress toward it; the branch's post-audit state was not reverified for this publication correction. This historical observation does not establish that the work is merged, current, or public.
