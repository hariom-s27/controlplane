...
| Tool arguments | UNTRUSTED | Typed `ProposedAction`; selected fields checked by predicates | No complete tool-schema or semantic validation; recipient binding is missing |
...
| T33 Authorization/recipient mismatch | SECURITY | Document confidentiality | `recipient_id` differs from authenticated/acting subject | Entitlement resolver checks `session.subject_id` | `entitlements.py`; `agents/knowledge_assistant.py` | OPEN ENGINEERING BUG | Execution sends to argument `recipient_id`; no predicate binds it to `session.subject_id`, and session identity itself is unauthenticated |
| T34 Cross-tenant access attempt | SECURITY | Tenant data | Agent proposes a document outside the acting subject's entitlement | Classification and customer entitlement are separately resolved and a contradiction blocks | `test_entitlement_check_blocks_the_cross_tenant_record`; `test_full_gate_blocks_the_cross_tenant_send` | PARTIALLY MITIGATED | Fixture uses matching subject/recipient; T10, T21, T29, and T33 leave reachable bypasses |
...
| Authorization binding | NO | Source inspection | Entitlement is checked for `session.subject_id` | Binding of execution `recipient_id` to subject identity is absent |
...
- Recipient mismatch -> entitlement checked for session subject -> implementation called with argument recipient -> a cross-recipient disclosure path remains open.
...
|---|---|---|
| P0 | T33 recipient/subject binding; T10/T29 knowledge-assistant outage fail-open | Direct source paths can disclose a document without checking the actual recipient or without available entitlement evidence |
...
The current implementation demonstrates several narrow safeguards on the governed, gate-enabled path: prose claims are separated from predicate facts; predicate uncertainty does not silently pass; missing and ambiguous evidence has explicit handling; invalid `MODIFY` payloads do not fall back to original arguments; and completed or indeterminate same-key executions are controlled within one process. It explicitly does not establish protection when callers bypass or disable the gate, when session identity or source data is wrong, when the knowledge-assistant outage posture permits execution, when recipient identity is not bound to the entitlement subject, after idempotency state is lost, or when a receipt key or host is compromised. There is no adversarial proof, deployment-boundary enforcement, or production-security certification.
