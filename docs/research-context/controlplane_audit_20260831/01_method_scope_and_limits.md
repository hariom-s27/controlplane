...
| C-2 | `docs/gold-set.md` §5 (branch `8a48bf7`): "`bench/seb1_exp5_confusion_matrix.py::run()` raises `SystemExit`" | `git diff origin/main origin/p03-m4…  -- bench/seb1_exp5_confusion_matrix.py` is **empty**; `run()` on the branch is byte-identical to main and returns a matrix | **Source wins.** The doc describes an intended state, not the branch's code. |
